...
def run_history(eq_series, oil_series, strategy: Strategy,
    ...
    Returns a list of HistoricalBar, equity rebased so the first non-zero bar
    matches initial_capital for rebalancing strategies, or left as raw dollars
    for BuyOnlyOilWarStrategy.
    """
    import pandas as pd
    ...
    # Rebase non-BuyOnly equity so bars[0].equity == initial_capital
    if not isinstance(strategy, BuyOnlyOilWarStrategy):
        base = bars[0].equity
        if base and base != 0:
            for b in bars:
                b.equity = round(b.equity / base * initial_capital, 2)
...
def simulate_future(eq_series, oil_series, strategy: Strategy,
    ...
    all_eq    = [[0.0] * forecast_days for _ in range(n_paths)]
    ...
    for p in range(n_paths):
        ...
        curr_equity = last_bar.equity

        strategy.reset()
        if isinstance(strategy, BuyOnlyOilWarStrategy):
            strategy.set_initial_capital(initial_capital)
            deployed_frac = last_bar.allocation
            if curr_eq > 0:
                fake_shares = (deployed_frac * initial_capital) / curr_eq
                strategy.record_purchase(fake_shares, deployed_frac * initial_capital)
        ...
        for d in range(forecast_days):
            ...
            if isinstance(strategy, BuyOnlyOilWarStrategy):
                ...
                curr_equity  = strategy.unrealized_value(new_eq)
                alloc_val    = strategy.cash_deployed / initial_capital
            else:
                alloc_val   = strategy.next_allocation(sigs)
                curr_equity = curr_equity * (1.0 + alloc_val * er)

            all_eq[p][d]    = curr_equity
    ...
    return ForecastPath(
        ...
        p10          = [round(float(v), 2) for v in np.percentile(eq_arr, 10, axis=0)],
        p25          = [round(float(v), 2) for v in np.percentile(eq_arr, 25, axis=0)],
        p50          = [round(float(v), 2) for v in np.percentile(eq_arr, 50, axis=0)],
        p75          = [round(float(v), 2) for v in np.percentile(eq_arr, 75, axis=0)],
        p90          = [round(float(v), 2) for v in np.percentile(eq_arr, 90, axis=0)],
